...
impl<'a> Parser<'a> {
    ...
    fn parse_ddl_create_table(&mut self) -> Result<ast::Statement> {
        ...
        Ok(ast::Statement::CreateTable { name: name, columns: columns })
    }
    ...
    fn parse_ddl_columnspec(&mut self) -> Result<ast::Column> {
        ...
        while let Some(Token::Keyword(keyword)) = self.next_if_keyword() {
            match keyword {
                ...
                Keyword::Default => {
                    column.default = Some(self.parse_expression(0)?);
                }
                Keyword::Unique => {
                    column.unique = true;
                }
                ...
                keyword => {
                    return Err(Error::Parse(format!("Unexpected keyword {}", keyword)));
                }
            }
        }
        Ok(column)
        ...
        match self.next()? {
            Token::Keyword(Keyword::Begin) => {
                ...
                if self.next_if_token(Keyword::As.into()).is_some() {
                    ...
                    match self.next()? {
                        Token::Number(n) => {
                            // version = Some(n.parse::<u64>()?);
                            version = Some(n.parse::<u64>().unwrap());
                        }
                        token => {
                            return Err(
                                Error::Parse(format!("Unexpected token {}, wanted number", token))
                            );
                        }
                    }
                }
        ...
        Ok(match self.next()? {
            ...
            Token::Number(n) => {
                if n.chars().all(|c| c.is_digit(10)) {
                    ast::Literal::Integer(n.parse().unwrap()).into()
                } else {
                    ast::Literal::Float(n.parse().unwrap()).into()
                }
            }
            Token::OpenParen => {
                let expr = self.parse_expression(0)?;
            ...
            t => {
                return Err(Error::Parse(format!("Expected expression atom, found {}", t)));
            }
        })
    }
}
